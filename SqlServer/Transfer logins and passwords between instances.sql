--1. Create stored procedures that will help generate necessary scripts to transfer logins and their passwords in Server A
USE [master]
  GO
...
--2.The output script that the sp_help_revlogin stored procedure generates is the login script. 
--This login script creates the logins that have the original Security Identifier (SID) and the original password.

EXEC sp_help_revlogin

-- Connect to Server B using any client tool (like SSMS) and then run the script generated (output of sp_helprevlogin) from Server A.
